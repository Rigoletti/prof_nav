...
const YovayshaLaResults = () => {
    ...
    useEffect(() => {
        if (location.state?.testResult) {
            ...
            setFinalScores(location.state.finalScores || location.state.testResult.yovayshaLaScores);
            setTopThreeTypes(location.state.topThreeTypes || location.state.testResult.topYovayshaLaTypes || []);
    ...
    const loadLatestResults = async () => {
        try {
            const response = await api.get('/tests/yovayshala/results');
            if (response.data.results && response.data.results.length > 0) {
                const latestResult = response.data.results[response.data.results.length - 1];
                setResults(latestResult);
                setFinalScores(latestResult.yovayshaLaScores);
                setTopThreeTypes(latestResult.topYovayshaLaTypes || []);
    ...
    const getSortedScores = () => {
        ...
        return Object.entries(finalScores)
            .sort((a, b) => b[1] - a[1])
            .map(([type, score]) => ({ type, score, info: YOVAYSHA_LA_TYPE_NAMES[type] }));
    };
    ...
    const getTypeIcon = (type) => {
        return YOVAYSHA_LA_TYPE_NAMES[type]?.icon || <PsychologyAltIcon />;
    };
    ...
    const primaryType = YOVAYSHA_LA_TYPE_NAMES[results.primaryYovayshaLaType];
            ...
            <Container maxWidth="lg">
                ...
                <Box sx={{ textAlign: 'center', mb: 8 }}>
                    ...
                    <Typography variant="h2" sx={{ mb: 2, fontWeight: 800 }}>
                        Ваш тип деятельности: {primaryType?.name}
                    </Typography>
                    ...
                    <Typography variant="h6" sx={{ color: 'text.secondary', mb: 2, maxWidth: 800, mx: 'auto' }}>
                        Топ-3 ваших предпочтений:
                    </Typography>
                    
                    <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, flexWrap: 'wrap', mb: 4 }}>
                        {topThree.map((item, index) => {
                            const typeInfo = YOVAYSHA_LA_TYPE_NAMES[item.type || item];
                        ...
                        <Card sx={{ borderRadius: 3, height: '100%' }}>
                            <CardContent sx={{ p: 4 }}>
                                <Typography variant="h6" sx={{ mb: 3, fontWeight: 600 }}>
                                    Результаты по типам деятельности
                                </Typography>
                            ...
                            <CardContent sx={{ p: 4 }}>
                                ...
                                <Box sx={{ mt: 4, p: 3, borderRadius: 2, backgroundColor: alpha(theme.palette.info.main, 0.05) }}>
                                    <Typography variant="body2" color="text.secondary">
                                        На основе ваших предпочтений мы определили соответствие типам личности по Климову.
                    ...
                    <CardContent sx={{ p: 4 }}>
                        ...
                        {recommendedSpecialties.length === 0 ? (
                            <Box sx={{ textAlign: 'center', py: 4 }}>
                                <PsychologyAltIcon sx={{ fontSize: 48, color: 'text.secondary', mb: 2, opacity: 0.5 }} />
                                <Typography variant="body1" color="text.secondary">
                                    Нет рекомендаций. Администратор еще не добавил специальности для вашего типа деятельности.
                                </Typography>
                            </Box>
                        ) : (
                    ...
                    <Grid container spacing={4} justifyContent="center">
                        <Grid item xs={12} sm={4}>
                            <Card sx={{ borderRadius: 3, p: 3, textAlign: 'center', height: '100%' }}>
                                <PsychologyAltIcon sx={{ fontSize: 40, color: 'primary.main', mb: 2 }} />
...
export default YovayshaLaResults;