...
const GOLOMShtok_TYPE_NAMES = {
    physics: { name: 'Физика и математика', color: '#3b82f6', icon: <CalculateIcon />, short: 'Ф' },
    chemistry: { name: 'Химия и биология', color: '#10b981', icon: <ScienceIcon />, short: 'Х' },
    electronics: { name: 'Радиотехника и электроника', color: '#8b5cf6', icon: <PsychologyIcon />, short: 'Р' },
    mechanics: { name: 'Механика и конструирование', color: '#f59e0b', icon: <BuildIcon />, short: 'М' },
    geography: { name: 'География и геология', color: '#06b6d4', icon: <PublicIcon />, short: 'Г' },
    literature: { name: 'Литература и искусство', color: '#ec4899', icon: <MenuBookIcon />, short: 'Л' },
    history: { name: 'История и политика', color: '#ef4444', icon: <HistoryIcon />, short: 'И' },
    pedagogy: { name: 'Педагогика и медицина', color: '#6366f1', icon: <SchoolOutlinedIcon />, short: 'П' },
    entrepreneurship: { name: 'Предпринимательство', color: '#f97316', icon: <BusinessCenterIcon />, short: 'Б' },
    sports: { name: 'Спорт и военное дело', color: '#6b7280', icon: <SportsSoccerIcon />, short: 'С' }
};

const GolomshtokResults = () => {
    ...
    useEffect(() => {
        if (location.state?.testResult) {
            ...
            setFinalScores(location.state.finalScores || location.state.testResult.golomshtokScores);
            setTopThreeTypes(location.state.topThreeTypes || location.state.testResult.topGolomshtokTypes || []);
    ...
    const loadLatestResults = async () => {
        try {
            const response = await api.get('/tests/golomshtok/results');
            if (response.data.results && response.data.results.length > 0) {
                const latestResult = response.data.results[response.data.results.length - 1];
                setResults(latestResult);
                setFinalScores(latestResult.golomshtokScores);
                setTopThreeTypes(latestResult.topGolomshtokTypes || []);
    ...
    const getSortedScores = () => {
        ...
        return Object.entries(finalScores)
            .sort((a, b) => b[1] - a[1])
            .map(([type, score]) => ({ type, score, info: GOLOMShtok_TYPE_NAMES[type] }));
    };
    ...
    const getTypeIcon = (type) => {
        return GOLOMShtok_TYPE_NAMES[type]?.icon || <PsychologyIcon />;
    };
    ...
    const primaryType = GOLOMShtok_TYPE_NAMES[results.primaryGolomshtokType];
            ...
            <Container maxWidth="lg">
                ...
                <Box sx={{ textAlign: 'center', mb: 8 }}>
                    ...
                    <Typography variant="h2" sx={{ mb: 2, fontWeight: 800 }}>
                        Ваши интересы: {primaryType?.name}
                    </Typography>
                    
                    <Typography variant="h6" sx={{ color: 'text.secondary', mb: 2, maxWidth: 800, mx: 'auto' }}>
                        Топ-3 сферы ваших интересов:
                    </Typography>
                    
                    <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, flexWrap: 'wrap', mb: 4 }}>
                        {topThree.map((item, index) => {
                            const typeInfo = GOLOMShtok_TYPE_NAMES[item.type || item];
                        ...
                        <Card sx={{ borderRadius: 3, height: '100%' }}>
                            <CardContent sx={{ p: 4 }}>
                                <Typography variant="h6" sx={{ mb: 3, fontWeight: 600 }}>
                                    Результаты по сферам интересов
                                </Typography>
                            ...
                            <CardContent sx={{ p: 4 }}>
                                ...
                                <Box sx={{ mt: 4, p: 3, borderRadius: 2, backgroundColor: alpha(theme.palette.info.main, 0.05) }}>
                                    <Typography variant="body2" color="text.secondary">
                                        На основе ваших интересов мы определили соответствие типам личности по Климову.
                    ...
                    <CardContent sx={{ p: 4 }}>
                        ...
                        {recommendedSpecialties.length === 0 ? (
                            <Box sx={{ textAlign: 'center', py: 4 }}>
                                ...
                                <Typography variant="body1" color="text.secondary">
                                    Нет рекомендаций. Администратор еще не добавил специальности для ваших интересов.
                                </Typography>
                            </Box>
                        ) : (
                        ...
                        <Grid item xs={12} sm={4}>
                            <Card sx={{ borderRadius: 3, p: 3, textAlign: 'center', height: '100%' }}>
                                ...
                                <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                                    Изучите описание ваших интересов и подходящие специальности
                                </Typography>
...
export default GolomshtokResults;