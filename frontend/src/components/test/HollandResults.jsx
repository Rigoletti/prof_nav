...
const HollandResults = () => {
    ...
    useEffect(() => {
        if (location.state?.testResult) {
            ...
            setFinalScores(location.state.finalScores || location.state.testResult.hollandScores);
            setTopThreeTypes(location.state.topThreeTypes || location.state.testResult.topHollandTypes || []);
    ...
    const loadLatestResults = async () => {
        try {
            const response = await api.get('/tests/holland/results');
            if (response.data.results && response.data.results.length > 0) {
                const latestResult = response.data.results[response.data.results.length - 1];
                setResults(latestResult);
                setFinalScores(latestResult.hollandScores);
                setTopThreeTypes(latestResult.topHollandTypes || []);
    ...
    const getSortedScores = () => {
        ...
        return Object.entries(finalScores)
            .sort((a, b) => b[1] - a[1])
            .map(([type, score]) => ({ type, score, info: HOLLAND_TYPE_NAMES[type] }));
    };
    ...
    const getTypeIcon = (type) => {
        return HOLLAND_TYPE_NAMES[type]?.icon || <PsychologyIcon />;
    };
    ...
    const primaryType = HOLLAND_TYPE_NAMES[results.primaryHollandType];
        ...
        <Box sx={{ 
            ...
            <Container maxWidth="lg">
                ...
                <Box sx={{ textAlign: 'center', mb: 8 }}>
                    ...
                    <Typography variant="h6" sx={{ color: 'text.secondary', mb: 2, maxWidth: 800, mx: 'auto' }}>
                        Топ-3 ваших профессиональных типа:
                    </Typography>
                    
                    <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, flexWrap: 'wrap', mb: 4 }}>
                        {topThree.map((item, index) => {
                            const typeInfo = HOLLAND_TYPE_NAMES[item.type || item];
                            ...
                            <CardContent sx={{ p: 4 }}>
                                ...
                                <Box sx={{ mt: 4, p: 3, borderRadius: 2, backgroundColor: alpha(theme.palette.info.main, 0.05) }}>
                                    <Typography variant="body2" color="text.secondary">
                                        На основе ваших профессиональных предпочтений мы определили соответствие типам личности по Климову.
