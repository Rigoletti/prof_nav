...
const YovayshaTest = () => {
    ...
    const startTest = useCallback(async () => {
        ...
        try {
            setLoading(true);
            const response = await api.get('/tests/yovaysha/start');
    ...
    const handleAnswer = async (action = 'next') => {
        ...
        try {
            const response = await api.post('/tests/yovaysha/answer', {
            ...
            if (response.data.success) {
                if (response.data.completed) {
                    navigate('/test/yovaysha/results', { 
                        state: { 
                    ...
                    <Paper sx={{ p: 2, borderRadius: 2, mb: 2, bgcolor: 'rgba(255,255,255,0.9)' }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            <Typography variant="h6" sx={{ fontWeight: 600 }}>
                                Тест Йовайши (Склонности к сферам деятельности)
                            </Typography>
                        </Box>
                    </Paper>
