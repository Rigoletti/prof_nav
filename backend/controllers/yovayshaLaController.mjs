// backend/controllers/yovayshaLaController.mjs
...
export const getYovayshaLaTest = async (req, res) => {
    try {
        ...
        let savedSession = await getSavedSession(userId, 'yovayshala');
        
        if (savedSession) {
            console.log('Found saved Yovaysha LA session, restoring...');
        ...
        // Сохраняем в БД
        await saveSessionToDB(userId, testSession, 'yovayshala');
    ...
    try {
        ...
        if (action === 'next') {
            ...
            if (updatedSession.answers.length >= TOTAL_QUESTIONS) {
                ...
                await TestSession.findOneAndDelete({ userId, testType: 'yovayshala', completed: false });
        ...
        await saveSessionToDB(userId, updatedSession, 'yovayshala');
